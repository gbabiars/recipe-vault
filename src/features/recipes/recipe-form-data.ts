...
export function parseRecipeFormData(formData: FormData): RecipeCreateInput {
  ...
  const candidate = {
    ...
    steps: Array.from(
      { length: Number.isInteger(stepCount) && stepCount > 0 ? stepCount : 0 },
      (_, index) => ({
        stepOrder: index + 1,
        instruction: optional(formData.get(`step-${index}-instruction`)),
...
export function formOptional(value: FormDataEntryValue | null) {
